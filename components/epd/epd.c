...
esp_err_t epd_io_init(const epd_config_t *config)
{
    ...
    if (config->skip_bus_init != (bool)false) {
        /* bus already up — skip initialisation */
    } else {
...
void epd_update(void)
{
    if (epd_type == (uint8_t)EPD370_UC8253) {
        ...
        /* cppcheck-suppress misra-c2012-15.5 */
        return;
    } else if (epd_type == (uint8_t)EPD154) {
        epd_write_reg(0x22U);
...
void epd_paint_showChar(uint16_t x, uint16_t y, uint16_t chr, uint16_t size1, uint16_t color)
{
    ...
    if ((uint16_t)(x_pos + size1) > EPD_H) {
        /* cppcheck-suppress misra-c2012-15.5 */
        return;
    }
    if ((uint16_t)(y_pos + (uint16_t)EPD_CHAR_WIDTH_8) > EPD_W) {
        /* cppcheck-suppress misra-c2012-15.5 */
        return;
    }
